...
import { getTodayEvents } from "../../Api/celebrationsApi";

const TodayCelebrations = ({ isVisible }) => {
    ...
    const fetchEvents = async () => {
      try {
        ...
        if (data.success) {
          ...
          const birthdayEvents = data.todayEvents.filter(
            (e) => e.type === "birthday"
          );
          const anniversaryEvents = data.todayEvents.filter(
            (e) => e.type === "anniversary"
          );

          setBirthdays(birthdayEvents);
          setAnniversaries(anniversaryEvents);
        }
      } catch (err) {
  ...
  const totalCelebrations = birthdays.length + anniversaries.length;

  if (totalCelebrations === 0) return null;
  ...
  const getDescriptionText = () => {
    ...
    if (anniversaries.length > 0)
      parts.push(
        `${anniversaries.length} anniversar${
  ...
  return (
    <div
      className={`mb-12 -mt-6 transition-all duration-1000 delay-300 ease-out ${
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {birthdays.map((person, index) => (
              ...
              >
                <div className="bg-gradient-to-br from-white to-rose-50/50 rounded-2xl p-5 border border-rose-100/50 shadow-lg hover:shadow-xl transition-all duration-500 relative overflow-hidden">
                    ...
                    <div className="relative">
                      <div className="w-16 h-16 rounded-2xl overflow-hidden shadow-lg group-hover:scale-105 transition-transform duration-300">
                        {person.avatar ? (
                          ...
                          <div className="flex items-center justify-center w-full h-full bg-gradient-to-br from-rose-400 to-pink-400 text-white font-bold text-xl">
                  ...
                  {/* <button className="w-full bg-gradient-to-r from-rose-500 to-pink-500 text-white py-3 px-4 rounded-xl font-medium hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 text-sm">
                    <Gift size={16} />
            ...
            {anniversaries.map((person, index) => (
              <div
                ...
                style={{
                  transitionDelay: `${(birthdays.length + index) * 100}ms`,
                }}
                className="group transition-all duration-500 hover:-translate-y-2"
              >
                <div className="bg-gradient-to-br from-white to-emerald-50/50 rounded-2xl p-5 border border-emerald-100/50 shadow-lg hover:shadow-xl transition-all duration-500 relative overflow-hidden">
                    ...
                    <div className="relative">
                      <div className="w-16 h-16 rounded-2xl overflow-hidden shadow-lg group-hover:scale-105 transition-transform duration-300">
                        {person.avatar ? (
                          ...
                          <div className="flex items-center justify-center w-full h-full bg-gradient-to-br from-emerald-400 to-teal-400 text-white font-bold text-xl">
                            {person.name?.charAt(0)}
                          </div>
                        )}
                      </div>
                  ...
                  {/* <button className="w-full bg-gradient-to-r from-emerald-500 to-teal-500 text-white py-3 px-4 rounded-xl font-medium hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 text-sm">
                    <Heart size={16} />
